-- Apex CRM 2.0 — visual workflow builder support
--
-- The builder's trigger panel offers a "from status" filter for
-- lead_status_change alongside the "to status" filter migration 0011
-- already matched — this extends handle_workflow_trigger_event() (full
-- create-or-replace, since Postgres has no partial function alter) to also
-- check trigger_config->>'from_status' against trigger_data's
-- previous_status. Everything else is byte-for-byte identical to 0011.
--
-- No table/column changes — the builder's condition (if/then) steps encode
-- their yes/no branches as extra keys on the step object itself
-- (yes_next_step_id / no_next_step_id) inside the existing `steps` jsonb
-- array, rather than needing new columns.
