...
// Trial length is 3 days. Thresholds below are tuned for that:
// - "urgent" (red) once 1 day or less remains
// - "warning" (amber) at 2 days remaining
// - "normal" (blue) only on day 3 (the first day)
const TOTAL_TRIAL_DAYS = 3;

function CountdownRing({ daysLeft, totalDays = TOTAL_TRIAL_DAYS }) {
  const radius = 16;
  const circumference = 2 * Math.PI * radius;
  const progress = Math.max(0, daysLeft / totalDays);
  const strokeDashoffset = circumference * (1 - progress);
  const color = daysLeft <= 1 ? "#E24B4A" : daysLeft <= 2 ? "#EF9F27" : "#185FA5";
...
export function TrialBanner({ daysLeft, plan, onUpgrade }) {
  ...
  // With a 3-day trial: urgent = last day (≤1), warning = 2 days left,
  // normal = only on the very first day (day 3)
    ...
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", fontFamily: "var(--font)" }}>
      <div style={{ textAlign: "center", maxWidth: 420, padding: "2rem" }}>
        ...
        <div style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.7, marginBottom: "2rem" }}>
          Your 3-day free trial is over. Upgrade to a paid plan to continue using Apex CRM — all your data is safe and waiting.
        </div>
